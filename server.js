...
async function getToken() {
  ...
  cache.set("token", token, 3600); // cache 1h
  return token;
}
...
app.get("/api/verifier", async (req, res) => {
  ...
  try {
    ...
    const inseeRes = await axios.get(`https://api.insee.fr/entreprises/sirene/V3/unites_legales/${siren}`, {
...
app.listen(port, () => {
  console.log("✅ API INSEE backend prête sur port " + port);
});
